...
export default function Footer() {
  ...
  const address = usePageContent("footer.address", "219A, No Trang Long, Binh Thanh, Ho Chi Minh City");
    ...
    <footer className="bg-gray-100 dark:bg-[#1a1a19] border-t border-gray-200 dark:border-[#3a3a38] mt-20 transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto px-6 py-14 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        <div>
          <div className="flex items-center gap-2 mb-4">
            ...
            >
              <span className="text-amber-900 font-display text-base font-black">FL</span>
            </div>
        ...
        <div>
          ...
          <ul className="space-y-3">
            <li className="flex items-start gap-2 text-sm text-gray-500 dark:text-[#7a7a78]">
              <MapPin size={15} className="mt-0.5 shrink-0 text-amber-500 dark:text-[#fdd52f]" />
              <EditableText settingKey="footer.address" defaultValue={address} />
            </li>
          ...
          <div className="mt-5 p-3 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-700/30 rounded-xl">
            <p className="text-xs text-amber-700 dark:text-amber-400 font-semibold">
              <EditableText settingKey="footer.officeHours" defaultValue={officeHours} />
            </p>
            <p className="text-xs text-gray-500 dark:text-[#7a7a78] mt-1 whitespace-pre-line">
              <EditableText settingKey="footer.hoursText" defaultValue={hoursText} multiline />
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-gray-200 dark:border-[#3a3a38] py-5 px-6">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-2 text-xs text-gray-400 dark:text-[#7a7a78]">
          <p><EditableText settingKey="footer.copyright" defaultValue={copyright} /></p>
          <div className="flex gap-4">
            <Link to="/privacy" className="hover:text-amber-600 dark:hover:text-white transition-colors">
              {f.privacy}
            </Link>
            <Link to="/faq" className="hover:text-amber-600 dark:hover:text-white transition-colors">
              {f.terms}
            </Link>
          </div>
        </div>
